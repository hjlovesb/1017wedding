...
function initCalendar() {
 ...
 /* ── Countdown ── */
  ...
  async function initStory() {
    ...
    window.setTimeout(() => {
      if (!reveal.classList.contains('is-playing')) play();
    }, 900);
  }
 ...
 function initGallerySlider() {
  ...
  gallery.innerHTML = `
    ...
    <button type="button" class="gallery__nav gallery__nav--prev" aria-label="이전 사진">‹</button>
    <button type="button" class="gallery__nav gallery__nav--next" aria-label="다음 사진">›</button>
...
(function () {
  ...
  function setupOurDayCountdown() {
    ...
    if (!countdown.querySelector('.our-day-countdown__title')) {
      const title = document.createElement('p');
      title.className = 'our-day-countdown__title';
      title.textContent = 'OUR DAY';
      countdown.prepend(title);
    }
    ...
    if (!countdown.querySelector('.our-day-countdown__message')) {
      ...
      message.innerHTML = `
        ...
        <img class="our-day-countdown__swan-inline" src="swan-divider.png" alt="하트 백조" />
